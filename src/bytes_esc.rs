...
struct ByteStrEscapes {
    start: u8,
    esc: u8,
    term: u8,
}
const BSTR_ESCAPE_ASC: ByteStrEscapes = ByteStrEscapes {
    start: 0xF8,
    esc: 0xFF,
    term: 0x01,
};
const BSTR_ESCAPE_DESC: ByteStrEscapes = ByteStrEscapes {
    start: !BSTR_ESCAPE_ASC.start,
    esc: !BSTR_ESCAPE_ASC.esc,
    term: !BSTR_ESCAPE_ASC.term,
};
...
#[inline]
pub fn deserialize_bytes_to_writer<P: EncodingParams>(
    ...
    ord_cond!(
        P,
        read_escaped_bytes_desc(reader, out),
...
#[cfg(feature = "std")]
pub fn deserialize_bytes_to_vec<P: EncodingParams>(
...
/// it is much faster.
pub fn serialize_bytes_noesc<P: EncodingParams>(
    mut writer: impl WriteBytes,
    v: &[u8],
    _param: P,
) -> Result {
...
pub fn deserialize_bytes_noesc_to_writer<P: EncodingParams>(
    mut reader: impl ReadBytes,
    mut writer: impl WriteBytes,
    _param: P,
) -> Result {
...
#[cfg(feature = "std")]
pub fn deserialize_bytes_noesc_to_vec<P: EncodingParams>(
    mut reader: impl ReadBytes,
    _param: P,
) -> Result<Vec<u8>> {
    ...
    ord_cond!(
        P,
        {
            write_complement_bytes(&mut res, v)?;
        },
        {
            res.write(v)?;
        }
...
#[cfg(feature = "std")]
pub fn deserialize_bytes_noesc_to_string<P: EncodingParams>(
    reader: impl ReadBytes,
    param: P,
) -> Result<String> {
