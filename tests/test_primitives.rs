...
use ordcode::{bytes_esc, params::*, primitives::SerializableValue, *};
use std::{f32, f64};
    ...
    ($t:ty, $sfn:ident, $dfn:ident, $tvs:ident) => {
        ...
        fn $sfn() {
            for val in $tvs {
                ...
                val.to_writer(&mut bb, AscendingOrder).unwrap();
                val.to_writer(WriteToTail(&mut bb), DescendingOrder)
                    .unwrap();
                let nl = bb.finalize().unwrap();
                let mut r = DeBytesReader::new(&buf[..nl]);
                assert_eq!(<$t>::from_reader(&mut r, AscendingOrder).unwrap(), *val);
                assert_eq!(
                    <$t>::from_reader(ReadFromTail(&mut r), DescendingOrder).unwrap(),
                    *val
                );
            }
        }
...
#[cfg(not(no_i128))]
test_ser!(u128, serialize_u128, deserialize_u128, V_U128);
#[cfg(not(no_i128))]
test_ser!(i128, serialize_i128, deserialize_i128, V_I128);
    ...
    ($tn:ident, $t:ty, $sf:ident, $tvs:ident) => {
        ...
        fn $tn() {
            fn encode_asc(v: $t) -> Vec<u8> {
                let mut s = vec![];
                v.to_writer(&mut s, AscendingOrder).unwrap();
                s
            }
            fn encode_desc(v: $t) -> Vec<u8> {
                let mut s = vec![];
                v.to_writer(&mut s, DescendingOrder).unwrap();
                s
            }
            for v1 in $tvs {
                for v2 in $tvs {
...
#[cfg(not(no_i128))]
test_cmpi!(cmp_u128, u128, serialize_u128, V_U128);
#[cfg(not(no_i128))]
test_cmpi!(cmp_i128, i128, serialize_i128, V_I128);
...
fn test_esc_enclen_desc() {
    let v = vec![
        0, 0, 0xF8, 3, 1, 0, 7, 0xFF, 0xF8, 0xFE, 1, 2, 7, 0, 1, 0xFE,
    ];
    let mut s = vec![];
...
fn cmp_esc_bytes_nested(param: impl EncodingParams) {
    let data = vec![
        0, 0u8, 0, 0xFF, 0xF8, 7, 3, 1, 0, 0xFF, 0, 0xFE, 1, 2, 3, 0, 1, 0xF1, 0xF1, 0xFF, 0xF1,
        0x01, 0x0E, 0x00, 0x0E, 0xFE,
    ];
    let mut s1 = vec![];
    let mut s2 = vec![];
